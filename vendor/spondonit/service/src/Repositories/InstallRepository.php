...
class InstallRepository
{
    ...
    public function validateDatabase($params)
    {
        if (config('spondonit.support_multi_connection', false)) {
            // $db_connection = gv($params, 'db_connection', env('DB_CONNECTION','pgsql'));
            $db_connection = gv($params, 'db_connection', 'pgsql');
        } else {
            // $db_connection = 'mysql';
            $db_connection = gv($params, 'db_connection', 'pgsql');
            // $db_connection = 'pgsql';
        ...
        $db_port = gv($params, 'db_port', 5432);
        $db_username = gv($params, 'db_username', 'postgres');
        $db_password = gv($params, 'db_password', 'admin');
        $db_database = gv($params, 'db_database', 'sanjeevinimart');
        // $db_host =  'localhost';
        // $db_port = 5432;
        // $db_username = 'postgres';
        // $db_password = 'admin';
        // $db_database = 'sanjeevinimart';
    ...
    {
        $db_connection = config('spondonit.support_multi_connection', false) ? gv($params, 'db_connection', 'pgsql') : 'pgsql';
        envu([
            'APP_URL' => app_url(),
